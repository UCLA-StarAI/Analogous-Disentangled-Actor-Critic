...
from mems.StepMemoryWithSeqRewardOnlyDoubleReward import StepMemoryWithSeqRewardOnlyDoubleReward
from mems.StepMemoryWithSeqRewardOnlyHEDoubleReward import StepMemoryWithSeqRewardOnlyHEDoubleReward
...
class EliteStepMemoryDDPGDoubleReward():
    ...
    def append(self, state, action, reward1, reward2, done):
        self.episode_buffer.append((state, action, reward1, reward2, done))
        self.current_score += reward1
    ...
    def episode_to_step_memory(self, episode_buffer):
        if self.mem_type == 0:
            mem = StepMemoryWithSeqRewardOnlyDoubleReward(max_buffer_size = len(self.episode_buffer),
                                                          device = self.device)
        elif self.mem_type == 1:
            mem = StepMemoryWithSeqRewardOnlyHEDoubleReward(max_buffer_size = len(self.episode_buffer),
                                                            device = self.device)

        for item in episode_buffer:
            mem.append(item[0], item[1], item[2], item[3], item[4])
        mem.append(item[0], item[1], item[2], item[3], item[4])
