...
class StackedTemporalMemoryPPO():
    def __init__(self, env_num, mem_step_len, state_shape, action_type,
        ...
        self.action_log_prob_buffer = np.zeros([mem_step_len + 1, env_num, 1], dtype = np.float32)

    def append(self, state, action, reward, done, log_prob):
        ...
        self.action_log_prob_buffer[self.step_idx, :, :] = np.expand_dims(deepcopy(log_prob), axis = -1)
    ...
    def sample(self, to_tensor = True):
        ...
        action_log_prob_batch = self.action_log_prob_buffer[:-1, :, :].copy()
        ...
        if to_tensor:
            if self.device is not None:
                ...
                action_log_prob_batch = torch.tensor(action_log_prob_batch, dtype = torch.float32).to(self.device)
            else:
                if isinstance(self.state_buffer, dict):
                ...
                action_log_prob_batch = torch.tensor(action_log_prob_batch, dtype=torch.float32)
        ...
        self.action_log_prob_buffer[0, :, :] = self.action_log_prob_buffer[-1, :, :]

        if isinstance(self.state_buffer, dict):
            return (img_state_batch, vec_state_batch), action_batch, reward_batch, done_batch, action_log_prob_batch
        else:
            return state_batch, action_batch, reward_batch, done_batch, action_log_prob_batch
